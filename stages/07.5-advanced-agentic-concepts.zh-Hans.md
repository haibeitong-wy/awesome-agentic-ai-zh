...
> 📋 **本章组成**：为什么有这 stage → **核心 mental model：Types → Config → Repo → Service 四层工作边界**（diagram）→ 12 个进阶概念 skeleton → 为什么选这 12 个 → **核心 Harness Engineering 原则（多 source、4 大类别 + 关系图）** → 进阶 agentic 应用流程（5 step）→ 完整 reading path → 自我检查
...
把 agent 当作新进实习生来想：你交代一个明确的小任务、他却自作主张把邻近的东西也动了——这就叫「跨工作边界」。`agent-collab-skills` 累积 dogfood 出 3 个真实案例（[完整记录](https://github.com/WenyuChiou/agent-collab-skills/blob/main/docs/observed-failure-modes.md)）：

- **F11（越界没收手）**：请 codex 做字符串替换 `"默认" → "预设"` 扫全工程、它顺手把那份「**示范这条规则本身**」的对照表也改了。等于实习生改错字、结果把字典里示范那个字的范例也改掉、把规则本身的教材给污染了。
- **F12（加料）**：同样请 codex 做字符串替换、它却擅自在多份文件末尾加「Attribution: Karpathy / Simon Willison / ...」这些没人要的署名。等于实习生觉得加注是好意、但加进别人的章节、又是没被要求的动作 = 越权。
- **F14（知道规则但跳过）**：规则写了「动到多语言文件就要跑 mirror-sync 检查」、但操作者觉得「这次很简单、跳过」就 commit 了。结果 ship 出三个语言版本对不齐。这比 F11/F12 更难防——错不在 agent、在操作者选择不按 SOP 走。

**这 3 个案例告诉你的事**：

- agent 不会「刚好停在你交代的那个点」——brief 要明确写「**只能动 X、绝对不要动 Y**」
- 规则「装好了 ≠ 会被遵守」——必须靠机械强制（git hook / acceptance-gate preset）才挡得住人类的「这次跳过一下」

→ **实作**：工作边界写进 brief（用 [`agent-collab-skills`](https://github.com/WenyuChiou/agent-collab-skills) 的 task-splitter pre-task scope confirmation）+ enforce 在 acceptance gate preset + mandatory rules 加 hook（不能只是 prose）。

### 🔁 Failure-mode lifecycle（F11-F14 怎么进化的）

![Failure-Mode Evolution Cycle](../resources/diagrams/failure-lifecycle.zh-Hans.png)

F11 / F12 / F13 / F14 不是随机编号，而是 [`agent-collab-skills` 累积观察的 failure modes](https://github.com/WenyuChiou/agent-collab-skills/blob/main/docs/observed-failure-modes.md)。每一个都走过 **发现 → 文档化 → encode 成 check → 自动消除** 的循环（见上图五步）。

**F14（skipping mandatory preset）走完整循环的真实案例**：

1. **事件**：2026-05-13、Phase D 操作者跳过 preset、shipped 出 mirror drift
2. **文档化**：`observed-failure-modes.md` 命名为 F14（META-FAILURE 类别）
3. **codify 成 check**：写进 `multi-locale-mirror-sync.yml` + `cross_document_link_text_parity` check（v0.2.3）
4. **回溯测试**：对 commit `4a60742` 跑、抓到 9+ real drift（6 个原 audit 漏的、3 个 audit 已知的）
5. **强制规则**：CLAUDE.md Complex Task Protocol 写进「trigger 触发时 preset 必跑」
...
1. **建立 mental model — 四层工作边界**：Types → Config → Repo → Service。想清楚 agent 能动到 stack 哪一层、跨层算违规。
   → 本 stage §四层工作边界（核心 mental model）
