...
Think of an agent like a new intern: you give them a clear, narrow task, and they take it on themselves to touch nearby things too — that's a "work-boundary violation". `agent-collab-skills` has accumulated 3 real cases from its dogfood ([full log](https://github.com/WenyuChiou/agent-collab-skills/blob/main/docs/observed-failure-modes.md)):

- **F11 (didn't stop at the boundary)**: We asked codex to run a string sweep ("默認" → "預設") across the project. It also rewrote the *example table that documented the rule itself*. Like an intern fixing a typo and accidentally editing the dictionary entry that demonstrates that exact word — they contaminated the reference material that defined the rule.
- **F12 (added unrequested extras)**: Same scope (a string sweep), but codex inserted "Attribution: Karpathy / Simon Willison / ..." lines into multiple files on its own. An intern adding citations is well-intentioned, but adding them in someone else's chapter on no one's request = overreach.
- **F14 (knew the rule, skipped it anyway)**: The rule said "any commit touching multi-locale files must run the mirror-sync check". The operator decided "this one is simple, I'll skip it" and committed. Result: three locale versions shipped out of sync. Harder to prevent than F11/F12 — the fault isn't the agent's, it's the operator choosing not to follow the SOP.

**What these 3 cases tell you**:

- Agents do not "naturally stop at the point you assigned" — your brief must explicitly say "**only touch X, do NOT touch Y**"
- A rule "being installed ≠ being followed" — you need mechanical enforcement (git hook / acceptance-gate preset) to block the human "I'll just skip it this once".

→ **In practice**: write the work boundary into the brief (using [`agent-collab-skills`](https://github.com/WenyuChiou/agent-collab-skills)' task-splitter pre-task scope confirmation), enforce it at the acceptance gate preset, and back mandatory rules with a hook (prose alone isn't enough).

### 🔁 Failure-mode lifecycle (how F11-F14 evolved)

![Failure-Mode Evolution Cycle](../resources/diagrams/failure-lifecycle.en.png)

F11 / F12 / F13 / F14 are not random labels. They are [`agent-collab-skills` observed failure modes](https://github.com/WenyuChiou/agent-collab-skills/blob/main/docs/observed-failure-modes.md), and each one goes through the same loop shown above: **discover → document → encode as a check → eliminate automatically**.

**Example: F14 (skipping a mandatory preset) through the full cycle**

1. **Incident**: 2026-05-13, Phase D operator skipped the preset and shipped a mirror drift
2. **observed-failure-modes.md**: named F14 (META-FAILURE category)
3. **Codify**: written into `multi-locale-mirror-sync.yml` + `cross_document_link_text_parity` check (v0.2.3)
4. **Retrospective test**: ran against commit `4a60742`, caught 9+ real drifts (6 missed by audit, 3 already known)
5. **Mandatory rule**: CLAUDE.md Complex Task Protocol gains "if the trigger fires, the preset must run"
...
1. **Build the mental model — four work-boundary layers**: Types → Config → Repo → Service. Decide which stack layer the agent may touch and when crossing a layer is a violation.
   → This stage §four-layer work boundary (core mental model)
