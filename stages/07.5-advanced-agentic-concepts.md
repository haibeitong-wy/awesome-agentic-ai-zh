...
> 📋 **本章組成**：為什麼有這 stage → **核心 mental model：Types → Config → Repo → Service 四層工作邊界**（diagram）→ 12 個進階概念 skeleton → 為什麼選這 12 個 → **核心 Harness Engineering 原則（多 source、4 大類別 + 關係圖）** → 進階 agentic 應用流程（5 step）→ 完整 reading path → 自我檢查
...
把 agent 當成新進實習生來想：你交代一個明確的小任務、他卻自作主張把鄰近的東西也動了——這就叫「跨工作邊界」。`agent-collab-skills` 累積 dogfood 出 3 個真實案例（[完整紀錄](https://github.com/WenyuChiou/agent-collab-skills/blob/main/docs/observed-failure-modes.md)）：
...
1. **建立 mental model — 四層工作邊界**：Types → Config → Repo → Service。想清楚 agent 能動到 stack 哪一層、跨層算違規。
   → 本 stage §四層工作邊界（核心 mental model）
